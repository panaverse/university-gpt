...
from app.crud.topic_crud import topic_crud, content_crud
...
async def new_topic(async_db_session):
    async for session in async_db_session:
        ...
        new_topic_created = await topic_crud.create_topic(topic=topic_data, db=session)
        yield new_topic_created
        # We will delete Topic after in the last test case instead
        await topic_crud.delete_topic(id=new_topic.id, db=session)
...
class TestTopicCRUD:
    ...
    async def test_read_topics(self, async_db_session):
        async for session in async_db_session:
            topics = await topic_crud.read_topics(offset=0, limit=10, db=session)
    ...
    async def test_get_topic_by_id(self, async_db_session, new_topic):
        async for session in async_db_session:
            async for topic in new_topic:
                fetched_topic = await topic_crud.read_topic_by_id(
                    id=topic.id, db=session
                )
        ...
        async for session in async_db_session:
            ...
            async for topic in new_topic:
                updated_topic = await topic_crud.update_topic(
                    id=topic.id, topic=topic_update, db=session
                )
    ...
    async def test_delete_topic(self, async_db_session, new_topic):
        async for session in async_db_session:
            async for topic in new_topic:
                await topic_crud.delete_topic(id=topic.id, db=session)
                deleted_topic = await topic_crud.get_topic_by_id(
                    id=topic.id, db=session
                )
    ...
    async def test_create_topic_value_error(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(ValueError):
                ...
                await topic_crud.create_topic(topic=topic, db=session)
    ...
    async def test_update_topic_value_error(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(ValueError):
                ...
                await topic_crud.update_topic(id=9999, topic=topic, db=session)
    ...
    async def test_delete_topic_http_exception(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(HTTPException):
                await topic_crud.delete_topic(id=9999, db=session)
    ...
    async for session in async_db_session:
        async for topic in new_topic:
            content_data = ContentCreate(
                topic_id=topic.id, content_text="Learn about classes in OOP"
            )
            new_content = await content_crud.create_new_content(
                content=content_data, db=session
            )
            yield new_content

            await content_crud.delete_content(id=new_content.id, db=session)
...
class TestContentCrud:
    ...
    async def test_read_content_for_topic(self, async_db_session, new_mock_content):
        async for session in async_db_session:
            async for mock_content in new_mock_content:
                content = await content_crud.read_content_for_topic(
                    topic_id=mock_content.topic_id, db=session
                )
    ...
    async def test_read_content_for_topic_non_existing_topic_id(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(HTTPException):
                await content_crud.read_content_for_topic(topic_id=9999, db=session)
    ...
    async def test_get_content_by_id(self, async_db_session, new_mock_content):
        async for session in async_db_session:
            async for mock_content in new_mock_content:
                content = await content_crud.get_content_by_id(
                    topic_id=mock_content.topic_id,
    ...
    async def test_get_content_by_id_non_existing_topic_id(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(HTTPException):
                await content_crud.get_content_by_id(
                    topic_id=9999, content_id=9999, db=session
                )
    ...
    async def test_update_content(self, async_db_session, new_mock_content):
        async for session in async_db_session:
            ...
            async for mock_content in new_mock_content:
                updated_content = await content_crud.update_content(
                    id=mock_content.id, content=content, db=session
                )
    ...
    async def test_update_content_non_existing_topic_id(self, async_db_session):
        async for session in async_db_session:
            with pytest.raises(HTTPException):
                content = ContentUpdate(content_text="New Classes")
                await content_crud.update_content(id=9999, content=content, db=session)
