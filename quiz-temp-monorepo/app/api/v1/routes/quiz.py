...
from app.crud.quiz_crud import quiz_engine, quiz_question_engine, quiz_setting_engine
...
async def create_new_quiz_setting(quiz_setting: QuizSettingCreate, db: AsyncSessionDep):
    """
    Create a new QuizSetting
    """
    try:
        return await quiz_setting_engine.create_quiz_setting(
...
async def get_all_quiz_settings_endpoint(quiz_id: int, db: AsyncSessionDep):
    """
    Get all QuizSettings
    """
    try:
        return await quiz_setting_engine.get_all_quiz_settings_for_quiz(
...
async def get_quiz_setting_by_id_endpoint(quiz_setting_id: int, db: AsyncSessionDep):
    """
    Get a QuizSetting by ID
    """
    try:
        return await quiz_setting_engine.get_quiz_setting_by_id(
...
async def update_quiz_setting_endpoint(
    quiz_setting_id: int, quiz_setting_update: QuizSettingUpdate, db: AsyncSessionDep
):
    """
    Update a QuizSetting
    """
    try:
        quiz_setting = await quiz_setting_engine.update_quiz_setting(
            db=db,
...
# REMOVE QUIZ SETTING
@router_quiz_setting.delete("/{quiz_setting_id}")
async def remove_quiz_setting_endpoint(quiz_setting_id: int, db: AsyncSessionDep):
    """
    Remove a QuizSetting
    """
    try:
        return await quiz_setting_engine.remove_quiz_setting(
            db=db, quiz_setting_id=quiz_setting_id
        )
    except Exception as err:
...
async def create_new_quiz(quiz: QuizCreate, db: AsyncSessionDep):
    """
    Create a new Quiz
    ...
    try:
        gen_quiz = await quiz_engine.create_quiz(quiz=quiz, db=db)
        return gen_quiz
...
async def call_read_all_quizzes(
    course_id: int,
    db: AsyncSessionDep,
    ...
    try:
        return await quiz_engine.read_all_quizzes_for_course(
...
async def call_read_quiz_by_id(quiz_id: int, db: AsyncSessionDep):
    """
    Read a Quiz by ID
...
async def update_existing_quiz(quiz_id: int, quiz: QuizUpdate, db: AsyncSessionDep):
    """
    ...
    try:
        return await quiz_engine.update_quiz(
...
async def delete_existing_quiz(quiz_id: int, db: AsyncSessionDep):
    """
...
async def create_new_quiz_question(
    ...
    try:
        return await quiz_question_engine.create_quiz_question(
...
async def call_remove_quiz_question(
    quiz_id: int, quiz_question_id: int, db: AsyncSessionDep
):
    """
    ...
    try:
        return await quiz_question_engine.remove_quiz_question(
