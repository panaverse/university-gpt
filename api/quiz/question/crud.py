...
async def add_question(question: QuestionBankCreate, db: AsyncSession):
    ...
    try:
        ...
        logger.error(f"ADD_Question: An integrity error occurred while adding the Question: {e}")
        raise HTTPException(status_code=500, detail="Data integrity issue.") 

    except SQLAlchemyError as e:
        await db.rollback()
        logger.error(f"ADD_Question: A database error occurred while adding the Question: {e}")
...
async def read_questions(db: AsyncSession, offset: int, limit: int):
    ...
    try:
        ...
        logger.error(f"READ_Questions: A database error occurred while reading the Questions: {e}")
        await db.rollback()
        raise HTTPException(status_code=500, detail="Database operation failed.")
    except Exception as e:
...
async def read_questions_by_type(question_type: str, db: AsyncSession ):
    ...
    try:
        ...
        await db.rollback()  # Ensure rollback is awaited
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No questions found") from e
...
async def update_question(id: int, question: QuestionBankUpdate, db: AsyncSession):
    ...
    try:
        ...
        if not question_to_update:
            raise ValueError(status_code=status.HTTP_404_NOT_FOUND, detail="Question not found")
...
async def add_mcq_option(mcq_option: MCQOptionCreate, session: AsyncSession):
    ...
    try:
        ...
        logger.error(f"ADD_MCQ_Option: An integrity error occurred while adding the MCQ Option: {e}")
        raise HTTPException(status_code=500, detail="Data integrity issue.")
    except SQLAlchemyError as e:
        await session.rollback()
        logger.error(f"ADD_MCQ_Option: A database error occurred while adding the MCQ Option: {e}")
        raise HTTPException(status_code=500, detail="Database operation failed.")
    except Exception as e:
        await session.rollback()
...
async def read_mcq_options(db: AsyncSession, offset: int, limit: int ):
    try:
        ...
        logger.error(f"READ_MCQ_Options: A database error occurred while reading the MCQ Options: {e}")
        db.rollback()
        raise HTTPException(status_code=500, detail="Database operation failed.")
    except Exception as e:
...
async def get_mcq_option_by_id(id: int, db: AsyncSession):
    ...
    try:
        ...
        await db.rollback()
        logger.error(f"GET_MCQ_Option: A database error occurred while retrieving the MCQ Option: {e}")
        raise HTTPException(status_code=500, detail="Database operation failed.")
    except Exception as e:
        await db.rollback()
