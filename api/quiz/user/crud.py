...
async def create_student(student: Student, db: AsyncSession):
    try:
        ...
        logger.error(f"CREATE_STUDENT: An integrity error occurred while creating the student: {e}")
        raise HTTPException(status_code=500, detail= "Data integrity issue.") from e

    except SQLAlchemyError as e:
        await db.rollback()
        logger.error(f"CREATE_STUDENT: A database error occurred while creating the student: {e}")
...
async def delete_student(id: int, db: AsyncSession ):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting student")
