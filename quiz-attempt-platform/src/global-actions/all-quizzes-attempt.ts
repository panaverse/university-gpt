...
export type AllQuizAttempted = QuizAttemptType[];

export const allAttemptedQuizzes = async ({
  token,
}: {
  token: string;
}): Promise<AllQuizAttempted> => {
  try {
    const res = await fetch(
      `${process.env.QUIZ_ATTEMPT_ASSESMENT_URL}/api/v1/answersheet/all`,
      {
        method: "GET",
        headers: {
