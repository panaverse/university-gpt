...
from tests.utils.test_items import temp_quiz_data  # Assumed structure, define accordingly
...
def test_update_quiz(db: Session, quiz_id):
    new_name = "Updated Quiz" + str(random.randint(1000, 9999))
    quiz_update_data = QuizUpdate(quiz_title=new_name, add_topic_ids=[1])  # Assume topic_ids has multiple IDs
