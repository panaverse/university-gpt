...
from app.init_data import init_course_id  #  course ID is needed for questions

# Fixture to Get a topic ID
@pytest.fixture
def init_topic_id(db: Session):
    topic_name = "topic_id" + str(random.randint(1, 10000))
...
@pytest.fixture
def question_id(db: Session, init_topic_id):
...
def test_add_question(db: Session, init_topic_id):
    question_text = temp_question.get("question_text") + str(random.randint(1, 10000))
    question_data = QuestionBankCreate(
...
def test_get_questions_by_topic_error(db: Session, init_topic_id):
    with pytest.raises(HTTPException) as e:
        question_crud.get_questions_by_topic(topic_id=init_topic_id, db=db)