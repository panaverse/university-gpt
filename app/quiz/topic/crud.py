...
from app.quiz.topic.models import Topic, TopicCreate, TopicUpdate, Content, ContentCreate, ContentUpdate
...
async def create_topic(topic: TopicCreate, db: AsyncSession) -> Topic:
    ...
    try:
        ...
        if topic.contents:
            topic.contents = [Content.model_validate(
                content) for content in topic.contents]
...
async def delete_topic(id: int, db: AsyncSession):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting topic")
...
async def delete_content(id: int, db: AsyncSession):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting content")
