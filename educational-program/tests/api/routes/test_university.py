...
def test_get_single_university(client: TestClient):
...
def test_get_all_universities(client: TestClient):
...
def test_update_university(client: TestClient):
    ...
    response = client.patch(
        f"{settings.API_V1_STR}/university/1", json={"name": temp_uni_name}
    )
    assert response.status_code == 200
    assert response.json()["name"] == temp_uni_name


def test_delete_university(client: TestClient):
...
def test_create_university_duplicate(client: TestClient):
    # Create Uni
    response = client.post(f"{settings.API_V1_STR}/university", json=test_uni_data)
...
def test_get_university_invalid_id(client: TestClient):
...
def test_update_university_invalid_id(client: TestClient):
    response = client.patch(
...
def test_delete_university_invalid_id(client: TestClient):
...
def test_get_all_universities_invalid_page(client: TestClient):
...
def test_get_all_universities_invalid_per_page(client: TestClient):
