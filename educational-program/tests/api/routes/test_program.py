...
def university_id(db: Session):
    all_universities = university_crud.get_all_universities_db(
        db=db, offset=0, per_page=10
    )
...
def test_create_new_program(client: TestClient, university_id):
    updated_program_name = str(test_program_data.get("name")) + str(
        random.randint(1, 10000)
    )

    response = client.post(
        f"{settings.API_V1_STR}/program",
        json={"name": updated_program_name, "university_id": university_id},
    )
    assert response.status_code == 200
    assert response.json()["name"] == updated_program_name


def test_get_all_programs(client: TestClient):
    response = client.get(f"{settings.API_V1_STR}/program")
    assert response.status_code == 200
    assert response.json()["count"] >= 1
    assert response.json()["all_records"][0]["id"] is not None
...
def test_get_program_by_id(client: TestClient):
    # Get all first
    response = client.get(f"{settings.API_V1_STR}/program")
    assert response.status_code == 200
    assert response.json()["count"] >= 1
...
def test_update_program_by_id(client: TestClient, existing_program_id):
    updated_program_name = str(test_program_data.get("name")) + str(
        random.randint(1, 10000)
    )
...
def test_delete_program_by_id(client: TestClient, existing_program_id):
    response = client.delete(f"{settings.API_V1_STR}/program/{existing_program_id}")
    assert response.status_code == 200
    assert response.json() == {"message": "Program deleted"}


def test_get_program_invalid_id(client: TestClient):
    response = client.get(f"{settings.API_V1_STR}/program/100")
    assert response.status_code == 404
    assert response.json() == {"detail": "Program not found"}


def test_update_program_invalid_id(client: TestClient):
    response = client.patch(
        f"{settings.API_V1_STR}/program/100", json={"name": "Program of Temp Test"}
    )
    assert response.status_code == 404
    assert response.json() == {"detail": "Program not found"}


def test_delete_program_invalid_id(client: TestClient):
    response = client.delete(f"{settings.API_V1_STR}/program/100")
    assert response.status_code == 404
    assert response.json() == {"detail": "Program not found"}


def test_get_all_programs_invalid_page(client: TestClient):
...
def test_get_all_programs_invalid_per_page(client: TestClient):
