...
from app.init_data import course_name
...
def test_create_new_course(
    ...
    updated_course_name = test_program_data.get("name") + str(random.randint(1, 10000))
...
def test_get_course_invalid_id(
    client: TestClient
):
...
def test_update_course_invalid_id(
    ...
    response = client.patch(
        f"{settings.API_V1_STR}/course/100",
        json={"name": "Course of Temp Test"}
    )
    assert response.status_code == 404
    assert response.json() == {"detail": "Course not found"}

def test_delete_course_invalid_id(
    client: TestClient
):
...
def test_get_all_courses_invalid_page(
    client: TestClient
):
...
def test_get_all_courses_invalid_per_page(
    client: TestClient
):
